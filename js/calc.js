...
// 並び順はfoods.jsのv配列と同じ:
// [ビタミンD(µg), B1(mg), B2(mg), B6(mg), C(mg), Ca(mg), Mg(mg), 鉄(mg), 亜鉛(mg), カリウム(mg)]
// カリウムは推奨量が未設定のため目安量（男2500/女2000mg）を採用。
// ============================================================
export const MICROS = [
  ...
  { k: 'k', label: 'カリウム', unit: 'mg', foods: 'バナナ・さつまいも・アボカド・ほうれん草' },
];

// 性別×年齢区分（18-29 / 30-49 / 50-64 / 65+）の目標値テーブル
const MICRO_TARGETS = {
  m: {
    18: [8.5, 1.4, 1.6, 1.4, 100, 800, 340, 7.5, 11, 2500],
    30: [8.5, 1.4, 1.6, 1.4, 100, 750, 370, 7.5, 11, 2500],
    50: [8.5, 1.3, 1.5, 1.4, 100, 750, 370, 7.5, 11, 2500],
    65: [8.5, 1.3, 1.5, 1.4, 100, 750, 350, 7.5, 11, 2500],
  },
  f: {
    18: [8.5, 1.1, 1.2, 1.1, 100, 650, 270, 10.5, 8, 2000],
    30: [8.5, 1.1, 1.2, 1.1, 100, 650, 290, 10.5, 8, 2000],
    50: [8.5, 1.1, 1.2, 1.1, 100, 650, 290, 6.5, 8, 2000],  // 鉄は月経なし想定
    65: [8.5, 1.1, 1.2, 1.1, 100, 650, 280, 6.0, 8, 2000],
  },
};
